...
public class Sprite{
    ...
    protected int realHeigh;
    private SpriteSheetInterface sheet;
	...
	*/
    ...
    public Sprite(int size, int x, int y, SpriteSheetInterface sheet, int rw, int rh){
        SIZE = size;
    ...
    private void load(){
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                int pixelIndex = (j + x) + (i + y) * sheetSize;
                pixels[j + i * SIZE] = sheetPixels[pixelIndex];
            }
        }
    }
